...
def get_figure(dept='live'):
    ...
    cities = load_json('https://analytics.usa.gov/data/'+ dept + \
        '/top-cities-realtime.json')['data']
    ...
    df['text'] = df['name'] + '<br> Visitors: ' + df['visitors'] + '<br>Population ' + (df['pop']/1e6).astype(str)+' million'
...
def get_monthly_traffic(dept='live',stat='users'):
    total = load_json(GOV_URL + dept + '/top-traffic-sources-30-days.json')['totals'][stat]
...
def get_top_domains(dept='live',days='7'):
    ...
    return dict(
        y=[d['domain'] for d in data[:20]],
        x=[d['visits'] for d in data[:20]]
    )
